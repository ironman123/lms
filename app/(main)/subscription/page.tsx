            ...
            <main className="max-w-5xl mx-auto px-4 py-16 space-y-16">
                ...
                {Object.values(byExam).length === 0 ? (
                    <div className="text-center py-20">
                        <p className="text-muted-foreground font-bold">No packs available yet. Check back soon.</p>
                    </div>
                ) : (
                    Object.values(byExam).map(({ exam, bundles: examBundles }) => (
                        <section key={exam.id}>
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                />
                                <h2 className="text-2xl font-black text-foreground">{exam.name}</h2>
                            </div>
                                ...
                                {examBundles.map((bundle) => {
                                    ...
                                    return (
                                        <div
                                            key={bundle.id}
                                            className={cn(
                                                "bg-card rounded-3xl border p-6 flex flex-col gap-4 shadow-sm relative overflow-hidden",
                                        ...
                                        >
                                            ...
                                            <div>
                                                <h3 className="text-lg font-black text-foreground leading-tight pr-12">
                                                    {bundle.name}
                                                </h3>
                                                {bundle.description && (
                                                    <p className="text-sm text-muted-foreground mt-1.5">{bundle.description}</p>
                                                )}
                                            </div>
                ...
                {/* FAQ */}
                <section className="bg-card rounded-3xl border border-border p-8">
