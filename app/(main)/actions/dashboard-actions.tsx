...
export async function getDashboardOverview() {
    ...
    const examStats = [...examMap.values()].map(({ examId, examName, examSlug, sessions }) => {
        ...
        const trend = examScores.length >= 2
            ? examScores[examScores.length - 1] - examScores[0] > 0 ? "improving" : "declining"
            : "neutral";
...
export async function getExamDashboard(examId: string) {
    ...
    const weakSubjects = subjectStats.filter(s => s.accuracy < 70);
