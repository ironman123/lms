...
const OUTPUT_FILE = path.join(process.cwd(), "scripts", "kpsc-syllabuses-document-ai.json");
...
const MAX_EXAMS = 10;     // set to e.g. 5 for a test run
...
async function extractTextFromPDF(buffer: Buffer): Promise<string> {
    const [result] = await docAIClient.processDocument({
        name: process.env.DOCUMENT_AI_PROCESSOR_ID!,
        rawDocument: {
...
async function parseWithGemini(pdfUrl: string) {
    ...
    // ── Step 1: Extract raw text via Document AI ──────────────────────────
    console.log(`  Extracting text via Document AI...`);
    ...
    // ── Step 2: Detect format from text (cheap, no PDF needed now) ────────
...
${rawText.slice(0, 1000)}`
    ...
    // ── Step 3: Structure with Gemini Flash (text only) + Google Search ───
    const extractionModel = genAI.getGenerativeModel({
        model: "gemini-2.5-flash",
        tools: [{ googleSearch: {} } as any],
    });

    // Pass raw text instead of PDF — much cheaper and more reliable
