...
const OUTPUT_FILE = path.join(process.cwd(), "scripts", "kpsc-syllabuses-pdf-parse.json");
...
async function scrapeListingPage(): Promise<ScrapedLink[]> {
    console.log("📄 Fetching KPSC syllabus listing pages...");
    const links: ScrapedLink[] = [];
    const seenUrls = new Set<string>();

    let page = 0;
    let hasMorePages = true;
    const MAX_PAGES = 18; // Safety limit to prevent accidental infinite loops

    while (hasMorePages && page < MAX_PAGES)
    {
        const pageUrl = `${SYLLABUS_PAGE}?tid=All&page=${page}`;
        ...
        try
        {
            ...
            // If no rows are found, we've reached the end of the pagination
            if (rows.length === 0)
            {
                console.log(`   No more exams found on page ${page}. Scraping complete.`);
                break;
            }
            ...
            // If a page loads but all links were duplicates, it might be looping the last page
            if (newLinksOnPage === 0)
            {
                console.log(`   No new unique links on page ${page}. Breaking to prevent loop.`);
                break;
            }

            page++;
            await sleep(SCRAPE_DELAY_MS); // Be polite to the server between page loads

        } catch (error: any)
        {
            console.error(`   ❌ Failed to fetch page ${page}: ${error.message}`);
            break; // Stop paginating if the server throws a fit (e.g., 500 error)
        }
    }

    console.log(`  Found ${links.length} total unique PDF links across ${page} pages\n`);
    return links;
}
...
- Preserve the EXACT language of the source document.
- If topics are in Malayalam, keep them in Malayalam.
- If topics are in English, keep them in English.
- If topics are mixed, keep them mixed exactly as they appear.
- Do NOT translate anything under any circumstances.
- The earlier "General Knowledge" topics were translated — this is WRONG. Keep original language.
`;
...
async function parseWithGemini(pdfUrl: string) {
    ...
    // ── Extract text locally — free, no API ──────────────────────────────
    ...
    //     try
    //     {
    //         const fmtResult = await detectionModel.generateContent(
    //             `Classify this syllabus text format. Reply ONE word only:
    // - "structured" → numbered sections with bullet/keyword topics
    // - "tabular"    → marks distribution table first, then prose
    // - "prose"      → flowing paragraphs

    // TEXT:
    // ${rawText.slice(0, 1000)}`
    //         );
    //         const fmt = fmtResult.response.text().trim().toLowerCase();
    //         if (fmt.includes("tabular")) format = "tabular";
    //         else if (fmt.includes("prose")) format = "prose";
    ...
    // ── Structure with Gemini Flash + Google Search ───────────────────────
        ...
        syllabus: parsed.syllabus.map((s: any) => ({
            ...
            topics: (s.topics ?? [])
                .map((t: string) =>
                    t.trim()
                        .split(">")
                        .map((seg: string) => stripListPrefix(seg.trim()))
                        .filter(Boolean)
                        .join(" > ")
                )
                .filter(Boolean),
...
async function main() {
    ...
    console.log(
        `Processing ${toProcess.length} new PDFs (${processedUrls.size} already done)\n`
    );
